...
def data_enhance(method, train_data, train_labels):
    ...
    if method == 'noise':
        ...
        reverse = np.zeros(train_data.shape)
        for i in range(len(train_data)):
            reverse[i] = np.flipud(train_data[i])
        return reverse, train_labels
    ...
    elif method == 'mixup_v2':
        mix_num= 10
        mix_var = 0.05
        x_mixup = train_data.copy()
        y_mixup = train_labels.copy()
        for i in range(len(x_mixup)):
            weights = np.random.normal(0, mix_var, mix_num).reshape(1, -1)
            index = np.random.randint(0, len(x_mixup), mix_num)
            x_mixup[i] = x_mixup[i] * (1-np.sum(weights)) + np.dot(weights, train_data[index].transpose(1, 0, 2))
#             y_mixup[i] = y_mixup[i] * (1-np.sum(weights)) + np.dot(weights, train_labels[index])
        return x_mixup, y_mixup
...
def infer(model_name, inputs, csv_output):
    ...
    print('- done -')
    print('submission file locates at %s' % csv_output)
